...
def load_fine_tune_checkpoint(model, checkpoint_path, checkpoint_type,
  ...
  options = tf.train.CheckpointOptions(experimental_io_device="/job:localhost") # add
  ckpt.restore(
      checkpoint_path, options=options).expect_partial().assert_existing_objects_matched()
  ...
  with open("ok", 'w') as not_thing:
    with strategy.scope():
      with tf.compat.v2.summary.record_if(
        ...
        options = tf.train.CheckpointOptions(experimental_io_device="/job:localhost") # add
        ckpt.restore(latest_checkpoint, options=options)
        ...
        for _ in range(global_step.value(), train_steps,
          ...
          # add
          wandb.log({"steps_per_sec": steps_per_sec}, step=current_step)
          ...
          for key, val in logged_dict.items():
            ...
            wandb.log({key: val}, step=current_step) # add
...
def eager_eval_loop(
  ...
  for k in eval_metrics:
    ...
    wandb.log({k: eval_metrics[k]}, step=global_step.value())
...
def eval_continuously(
  ...
  for latest_checkpoint in tf.train.checkpoints_iterator(
    ...
    if eval_config.use_moving_averages:
      ...
      optimizer.shadow_copy(detection_model)
    options = tf.train.CheckpointOptions(experimental_io_device="/job:localhost") # add
    ckpt.restore(latest_checkpoint, options=options).expect_partial()
